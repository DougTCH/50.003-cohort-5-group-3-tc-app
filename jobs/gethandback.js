               ...
               for(var [k,p] of Object.entries(progs)){
                    ...
                    try{                    
                         ...
                         ,(err,output)=>{
                              if(err) throw err;
                              var date = new Date();
                              var fn =`./AccrualFiles/${k}_ACCRUAL_${date.getFullYear()}${date.getMonth()+1}${date.getDate()}.csv`;
                              fs.writeFileSync(fn,output);
                              SFTPService.Client.connect(SFTPService.Client.params).then(()=>{
                                   SFTPService.Client.uploadFile(fn,`/sutd_project_2024/c5g3/Accrual/${k}_ACCRUAL_${date.getFullYear()}${date.getMonth()+1}${date.getDate()}.csv`);
                                   //Need 'recipt'
                              });  
                         });
                    }catch(err){
